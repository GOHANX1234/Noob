...
// Get the path of the running .so itself
...
// Write data to another process memory
...
// Get the base address of the library from /proc/<pid>/maps
...
// Find offset of function in system library
...
// Inject the current .so into the process with pid
bool remote_inject(pid_t pid) {
    ...
    // ⚠️ No syscall execution yet to call dlopen from target thread
   // To actually call dlopen you need to fake callstack or remote thread
